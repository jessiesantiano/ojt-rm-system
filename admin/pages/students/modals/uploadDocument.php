<?php $students = mysqli_query($db, "SELECT * FROM students WHERE id=$id"); ?>
<?php while ($row = mysqli_fetch_array($students)) { ?>
  <div id="upload<?php echo $row['studentID']; ?>" class="show fade hidden">
			...
			<div class="w-2/5 h-f ll relative flex  flex-col  bg-white border border-solid pointer-events-auto dark:bg-gray-950 bg-clip-padding border-black/20 rounded-xl outline-0">
			 ...
			 <div class="py-6 px-6 lg:px-8">
                    <form class="space-y-6" action="process.php" method="post" enctype="multipart/form-data">
                      ...
                      <button type="submit" name="upload" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                        Upload
                      </button>
